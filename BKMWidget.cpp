...
void BKMWidget::showMenu_bkNameList(const QPoint& pos)
{
	...
	if (item) {
		...
		QAction* browseSourceAction = browseMenu->addAction(tr("浏览源文件路径"));
		QAction* browseBackupAction = browseMenu->addAction(tr("浏览存档路径"));

		QAction* editAction = menu.addAction(tr("编辑"));
		QAction* removeAction = menu.addAction(tr("移除"));
		QAction* deleteAction = menu.addAction(tr("删除存档"));
